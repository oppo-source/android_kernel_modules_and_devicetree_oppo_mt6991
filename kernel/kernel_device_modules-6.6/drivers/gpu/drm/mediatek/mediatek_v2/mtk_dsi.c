...
#endif
int debug_urgent_low = 17;
module_param(debug_urgent_low, int, 0644);
int debug_always_pre_ultra = 1;
module_param(debug_always_pre_ultra, int, 0644);
...
static void mtk_dsi_tx_buf_rw(struct mtk_dsi *dsi)
{
	...
	if (dsi->ext->params->is_cphy) {
		sodi_hi = tmp - (12 * (fill_rate - dsi->data_rate * 2 * dsi->lanes / 7
				/ buffer_unit) / 10);
		sodi_lo = (60 + 5) * dsi->data_rate * 2 * dsi->lanes / 7 / buffer_unit;
		preultra_hi = 71 * dsi->data_rate * 2 * dsi->lanes / 7 / buffer_unit;
		preultra_lo = 70 * dsi->data_rate * 2 * dsi->lanes / 7 / buffer_unit;
		ultra_hi = 61 * dsi->data_rate * 2 * dsi->lanes / 7 / buffer_unit;
		ultra_lo = 60 * dsi->data_rate * 2 * dsi->lanes / 7 / buffer_unit;
		...
		sodi_hi = tmp - (12 * (fill_rate - dsi->data_rate * dsi->lanes / 8
				/ buffer_unit) / 10);
		/*dsi->driver_data*/
		sodi_lo = (60 + 5) * dsi->data_rate * dsi->lanes / 8 / buffer_unit;
		if (debug_always_pre_ultra && priv->data->mmsys_id == MMSYS_MT6991 &&
			buf_con && comp->id == DDP_COMPONENT_DSI0) {
			preultra_hi = buf_con;
			preultra_lo = buf_con - 1;
		} else {
			preultra_hi = 71 * dsi->data_rate * dsi->lanes / 8 / buffer_unit;
			preultra_lo = 70 * dsi->data_rate * dsi->lanes / 8 / buffer_unit;
		}
		ultra_hi = 61 * dsi->data_rate * dsi->lanes / 8 / buffer_unit;
		ultra_lo = 60 * dsi->data_rate * dsi->lanes / 8 / buffer_unit;
		if (debug_urgent_low) {
			urgent_hi_fifo_us = debug_urgent_low + 1;
			urgent_lo_fifo_us = debug_urgent_low;
		}
...
#ifdef OPLUS_TRACKPOINT_REPORT
				display_exception_trackpoint_report("DisplayDriverID@@506$$ underrun MP4 24.11.15");
#endif
		}
