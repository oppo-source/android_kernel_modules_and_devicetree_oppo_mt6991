...
def _skip_build_checks(ctx, what):
    ...
    if ctx.attr._ubsan[BuildSettingInfo].value:
        print("\nWARNING: {this_label}: {what} was\
              IGNORED because --ubsan is set!".format(this_label = ctx.label, what = what))
        return True

    if ctx.attr._kocov[BuildSettingInfo].value:
        print("\nWARNING: {this_label}: {what} was\
              IGNORED because --kocov is set!".format(this_label = ctx.label, what = what))
        return True
