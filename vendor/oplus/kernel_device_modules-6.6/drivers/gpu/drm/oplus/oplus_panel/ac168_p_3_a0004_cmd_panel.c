...
#include "ac168_p_3_a0004_data_hw_roundpattern.h"
...
static int lcm_panel_ldo3_enable(struct device *dev)
{
	...
	if (ret < 0)
		OPLUS_DISP_ERR(TAG,"set voltage disp_ldo3 fail, ret = %d\n", ret);
	retval |= ret;

	/* enable regulator */
	ret = regulator_enable(disp_ldo3);
	if (ret < 0)
		OPLUS_DISP_ERR(TAG,"enable regulator disp_ldo3 fail, ret = %d\n", ret);
	retval |= ret;
	OPLUS_DISP_ERR(TAG,"get lcm_panel_ldo3_enable\n");
...
static int lcm_panel_ldo3_disable(struct device *dev)
{
	...
	if (ret < 0)
		OPLUS_DISP_ERR(TAG,"disable regulator disp_ldo3 fail, ret = %d\n", ret);
	retval |= ret;
	OPLUS_DISP_ERR(TAG,"disable regulator disp_ldo3\n");
...
static int oplus_panel_set_backlight_cmdq(void *dsi, dcs_write_gce_pack cb, void *handle, unsigned int level)
{
	...
	OPLUS_DISP_DEBUG(TAG,"level is %d\n",level);
	BL_LSB = level >> 8;
	BL_MSB = level & 0xFF;
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	OPLUS_DISP_INFO(TAG,"[LCM] %s+ ac168_p_3_a0004_cmd_panel Start\n", __func__);
...
//#endif /* OPLUS_FEATURE_DISPLAY */
	...
	OPLUS_DISP_INFO(TAG,"[LCM] %s+ ac168_p_3_a0004_cmd_panel End\n", __func__);
	return ret;
}
